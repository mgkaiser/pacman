...
# VICE emulators. 
VICE_HOME := "C:\C64\GTK3VICE-3.4-win64-r39109\bin\"

# On Windows machines CC65 emulators may not be available in the PATH by default.
CC65_HOME := C:\C64\cc65-snapshot-win32
...
c64_EMUCMD := $(VICE_HOME)x64sc -VICIIdsize +autostart-warp -model ntsc
c128_EMUCMD := $(VICE_HOME)x128 -VICIIdsize +autostart-warp -model ntsc
vic20_EMUCMD := $(VICE_HOME)xvic -VICdsize +autostart-warp -model ntsc
pet_EMUCMD := $(VICE_HOME)xpet -Crtcdsize +autostart-warp -model ntsc
plus4_EMUCMD := $(VICE_HOME)xplus4 -TEDdsize +autostart-warp -model ntsc
# So far there is no x16 emulator in VICE (why??) so we have to use xplus4 with -memsize option
c16_EMUCMD := $(VICE_HOME)xplus4 -ramsize 16 -TEDdsize +autostart-warp -model ntsc
cbm510_EMUCMD := $(VICE_HOME)xcbm2 -model 510 -VICIIdsize +autostart-warp -model ntsc
cbm610_EMUCMD := $(VICE_HOME)xcbm2 -model 610 -Crtcdsize +autostart-warp -model ntsc
...
# Diskname = Program without .c64
DISKNAME := $(PROGRAM).d64
...
$(C1541) -attach $(DISKNAME) -write "$(subst ?,$(SPACE),$(file))" $(notdir $(file)),p 

endef # D64_WRITE_PRG_recipe

define D64_WRITE_SEQ_recipe

$(C1541) -attach $(DISKNAME) -write "$(subst ?,$(SPACE),$(file))" $(notdir $(file)),s 
